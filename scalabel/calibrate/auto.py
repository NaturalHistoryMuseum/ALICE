...
import skimage.io
from skimage.morphology import remove_small_holes, remove_small_objects, skeletonize, closing, square, thin
...
class Point:
    ...
    def colour(self):
        ...
        return np.array([(channel[neighbourhood] * self.mask[neighbourhood]).sum() / self.mask[neighbourhood].sum()
...
def detect_corners(image, scale_factor):
    ...
    _, theta_peak, d_peak = hough_line_peaks(h, theta, d, min_distance=20, threshold=(0.1 * h.max()), num_peaks=4)

    lines = sorted(zip(theta_peak, d_peak), key=itemgetter(0))
    perpendicular_pairs = lines[:2], lines[2:]

    points = [Point(intersection(*lines), image, mask) for lines in itertools.product(*perpendicular_pairs)]
    return min(itertools.permutations(points), key=colour_difference)


image_names = ['0002_ALICE3.JPG', '0002_ALICE4.JPG', '0002_ALICE5.JPG', '0002_ALICE6.JPG']
images = [skimage.io.imread(Path() / 'data' / 'calibration' / 'pattern' / name) for name in image_names]
...
with open('square.csv', 'w+') as csvfile:
    ...
    for image_index, image_corners in enumerate(corners):
        for point_index, point in enumerate(image_corners):
            ...
            print(f'{image_index:<6} {point_index:<5} {int(x):<4} {int(y):<4}', file=csvfile)
