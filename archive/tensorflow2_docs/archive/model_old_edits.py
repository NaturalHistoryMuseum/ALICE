...
"""
ASJ Note: If things are not working on colab, try limiting GPU growth with:
"""
...
"""
ASJ Note: can use keras.utils.Sequence for more recent versions of tensorflow
but not older versions such as tensorflow==2.8.0. For older versions, we should
use keras.utils.all_utils.Sequence. Thus, we add a "try and except" and import earlier.
"""

try:
    Sequence__ = keras.utils.Sequence
except:
    Sequence__ = keras.utils.all_utils.Sequence


class DataGenerator(Sequence__):
    ...
    def data_generator(self, image_ids):
        ...
        while b < self.batch_size:
            try:
                ...
                if (
                    self.dataset.image_info[image_id]["source"]
                    in self.no_augmentation_sources
                ):
