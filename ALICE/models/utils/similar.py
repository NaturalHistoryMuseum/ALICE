...
class SimilarAsPossible:
    ...
    def __init__(self, shape, grid_separation):
        self.height, self.width = math.ceil(shape[0] / grid_separation[0]), math.ceil(
            shape[1] / grid_separation[1])
    ...
    def data_term(self, P, P_hat):
        ...
        A = coo_matrix(sparse_data_weights(P, (self.height + 1, self.width + 1),
                                           self.grid_separation),
    ...
    def triangle_indices(self, coords):
        ...
        return coo_matrix((np.ones(6), (np.arange(6), combined_indices)),
                          (6, self.num_vertices * 2))
    ...
    def shape_preserving_term(self):
        ...
        A = vstack(
            [(c - c1 - self.s * self.R90 * (c0 - c1)) * t for t in self.triangles])
    ...
    def fit(self, P, P_hat, alpha):
        ...
        self.transformation = estimate_transform('piecewise-affine', grid,
                                                 self.V.reshape(-1, 2))
...
def sparse_data_weights(P, grid_dimensions, grid_separation):
    ...
    for i in range(num_features):
        ...
        for vertex_index, weight in zip(
                *closest_corners(p, grid_dimensions, grid_separation)):
            for co in range(2):
                w[index] = weight
                row[index] = i * 2 + co
